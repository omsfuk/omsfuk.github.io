-------------------------------
title: 跨域解决方案（基于Spring MVC和AngularJS
tags: [CORS,Spring MVC,AngularJS]
-------------------------------
[TOC]
# 跨域解决方案（基于Spring MVC和AngularJS）

> 转载自阮老师的《[跨域资源共享 CORS 详解](http://www.ruanyifeng.com/blog/2016/04/cors.html)》
...
# 具体实践（基于Spring MVC 4、AngularJS 1.2.13）
## 以下是对阮老师博客内容的具体实践

具体到Sprig MVC中，就是增加一个过滤器
```java
...
$http({
	method: 'POST',
	url: 'http://172.25.157.29:8080/api/login',
	withCredentials: true, //一定要带上此属性，否则无法传递Cookie
...
另外，还碰到了一些坑。因为假设要用跨域方法调用登录的话，获得的`jsessionid`是不同源的，所以当利用`windows.href.location`进行页面跳转时，`jsessionid`以及其他异源`cookie`是无法传递的。涉及到此问题的具体解决方案，作为一只后端猿，我果断把它做成了单页面应用=_=。
